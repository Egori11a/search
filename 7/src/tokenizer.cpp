...
// кириллица в UTF-8: 2 байта, чаще 0xD0/0xD1
...
void Tokenizer::to_lower_cyrillic_pair(uint8_t& c1, uint8_t& c2) {
  // А(0xD0 0x90) .. Я(0xD0 0xAF) -> а(0xD0 0xB0) .. п(0xD0 0xBF)
  // Р(0xD0 0xA0) .. Я -> р(0xD1 0x80) .. я(0xD1 0x8F) (переход диапазона)
  // Ё(0xD0 0x81) -> ё(0xD1 0x91)
  if (c1 == 0xD0 && c2 == 0x81) { c1 = 0xD1; c2 = 0x91; return; } // Ё->ё
  if (c1 == 0xD0 && c2 >= 0x90 && c2 <= 0x9F) { c2 = (uint8_t)(c2 + 0x20); return; } // А..П
  if (c1 == 0xD0 && c2 >= 0xA0 && c2 <= 0xAF) { c1 = 0xD1; c2 = (uint8_t)(c2 - 0x20); return; } // Р..Я -> р..я
}

void Tokenizer::normalize_yo_pair(uint8_t& c1, uint8_t& c2) {
  // ё (0xD1 0x91) -> е (0xD0 0xB5)
  if (c1 == 0xD1 && c2 == 0x91) { c1 = 0xD0; c2 = 0xB5; }
}

int Tokenizer::utf8_token_len_chars(const std::string& s) {
  // считаем символы: ASCII 1 байт, кириллица 2 байта
  int n = 0;
...
void Tokenizer::tokenize(const std::string& text, std::vector<std::string>& out_tokens) const {
  ...
  for (size_t i = 0; i < text.size();) {
    ...
    if (c1 < 0x80) {
      ...
      // дефис/апостроф внутрь токена только если уже есть токен
      if ((is_dash(c1) || is_apos(c1)) && !cur.empty()) {
        // посмотрим, что дальше будет token-char
        if (i + 1 < text.size()) {
          uint8_t n1 = (uint8_t)text[i + 1];
          bool next_ok = is_ascii_letter(n1) || is_ascii_digit(n1);
          if (!next_ok && n1 >= 0x80 && i + 2 <= text.size()) next_ok = true; // кириллица начнётся парой
    ...
    // UTF-8 (ожидаем 2 байта кириллицы)
    if (i + 1 < text.size()) {
