...
// Russian Porter Stemmer (UTF-8, lowercase, ё->е expected)
class RussianStemmer {
public:
  std::string stem(const std::string& token) const;

private:
  // stem one word (no hyphen splitting)
  std::string stem_word(const std::string& w) const;

  // UTF-8 helpers (Cyrillic 2-byte sequences)
  ...
  // find RV and R2 boundaries by character index (not byte index)
  // returns byte offset where region starts
